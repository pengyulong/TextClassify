...
class LSIClassify(ProjectPath):
    def __init__(self,num_topics,column,docs,mode):
        ...
        self.docs = docs
        self.mode = mode
        self.lsi_model_file = os.path.join(self.model_dir,"lsi_{}_{}d.model".format(self.column,self.num_topics))
        self.lsi_data_file = os.path.join(self.data_dir,"lsi_{}_{}.dat".format(self.column,self.num_topics))
    ...
    def load_data(self):
        '''
        通过对输入的corpus进行加工,得到gensim能用的dictionary和corpus,用于后续训练lsi模型
    ...
    def train_lsi_model(self):
        ...
        :return: lsi模型
        '''
        if os.path.exists(self.lsi_model_file):
            logging.info('load lsi feature extractor model...')
            lsi = models.LsiModel.load(self.lsi_model_file)
        else:
            logging.info("train lsi feature extractor model...")
            #corpus_tfidf = tfidf[corpus]
            lsi = models.LsiModel(self.corpus, id2word=self.dictionary, num_topics=self.num_topics)
            lsi.save(self.lsi_model_file)
        self.model = lsi

    def _get_lsi_feature(self,corpus):
        '''
    ...
    def train_classify(self):
        ...
        :return:
        '''
        dataX,dataY = self._get_lsi_feature(self.trainSet[self.column].tolist()),(self.trainSet['class']-1).tolist()
    ...
    def predict(self):
        ...
        result_file = os.path.join(self.result_dir,"lsi_{}_{}_{}d_{:.3f}.csv".format(self.column,self.mode,self.num_topics,self.best_score))
        result_prob_file = os.path.join(self.result_dir,"lsi_{}_{}_{}d_{:.3f}_prob.csv".format(self.column,self.mode,self.num_topics,self.best_score))
...
if __name__ == "__main__":
    ...
    mode = "lightgbm"
    lsi_classify = LSIClassify(num_topics,column,corpus,mode)
    lsi_classify.load_data()
    lsi_classify.train_lsi_model()
    lsi_classify.save_features()
    lsi_classify.train_classify()
    lsi_classify.predict()

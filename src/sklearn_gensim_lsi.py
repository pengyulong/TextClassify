...
def load_data():
    ...
    for col in ['article','word_seg']:
        ...
        tfidf,lsi = train_lsi_model(corpus,dictionary,col)
    return True
...
if __name__ == "__main__":
    ...
    t1 = time.time()
    load_data()
    train_data_file = 'datadir/train_lsi_{}d.dat'.format(NUM_DIM)
    predX_file = 'datadir/predX_lsi_{}d.dat'.format(NUM_DIM)
    try:
        ...
        test_features = []
        for col in column:
            lsi = models.LsiModel.load('model/{}_{}_lsi.model'.format(col,NUM_DIM))
            dictionary = corpora.Dictionary.load('model/{}.dict'.format(col))
            train_features.append(get_lsi_feature(trainSet,col,lsi))
            val_features.append(get_lsi_feature(valSet,col,lsi))
            test_features.append(get_lsi_feature(testSet,col,lsi))
        logging.info("开始组合特征....")
        ...
        joblib.dump(train_data,train_data_file)
        joblib.dump(predX,predX_file)
    logging.info("训练{}分类模型".format(mode))
    logging.info("trainX'shape:{},trainY's shape:{}".format(trainX.shape,trainY.shape))
